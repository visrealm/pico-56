...
void slideshowScanline(uint16_t y, uint16_t pixels[VGA_VIRTUAL_WIDTH])
{
  static int frame = 0;
  ...
  if (transition & 0x01)
  {
    ...
    {
      memcpy(pixels, current + (y * VGA_VIRTUAL_WIDTH), VGA_VIRTUAL_WIDTH * sizeof(uint16_t));
    }
    else
    {
      memcpy(pixels, last + (y * VGA_VIRTUAL_WIDTH), VGA_VIRTUAL_WIDTH * sizeof(uint16_t));
    }
  }
  else
  {
    ...
    memcpy(pixels + offset, current + (y * VGA_VIRTUAL_WIDTH), (VGA_VIRTUAL_WIDTH - offset) * sizeof(uint16_t));
    memcpy(pixels, last + (y * (VGA_VIRTUAL_WIDTH)) - offset, offset * sizeof(uint16_t));
  }
